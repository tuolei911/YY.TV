...
namespace Data
{
    ...
    params object[] parameterValues)
      ...
      string commandText, params SqlParameter[] commandParameters)
        ...
        #region ExecuteBulkCopy -- by ws 140213
        public static void ExecuteBulkCopy(string connectionString, DataTable srcdata, string tableName)
        {
            if (connectionString == null || connectionString.Length == 0)
                throw new ArgumentNullException("connectionString");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                // Call the overload that takes a connection in place of the connection string
                ExecuteBulkCopy(connection, srcdata, tableName);
            }
        }

        public static void ExecuteBulkCopy(SqlConnection connection, DataTable srcdata, string tableName)
        {
            if (connection == null)
                throw new ArgumentNullException("connection");
            ...
            ExecuteNonQuery(transaction, srcdata, tableName);

            transaction.Commit();
        }

        public static void ExecuteNonQuery(SqlTransaction transaction, DataTable srcdata, string tableName)
        {
            if (transaction == null)
                throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null)
            ...
            using (SqlBulkCopy bulkCopy =
                       new SqlBulkCopy(transaction.Connection, SqlBulkCopyOptions.KeepIdentity, transaction))
            {
                bulkCopy.DestinationTableName = tableName;
                bulkCopy.WriteToServer(srcdata);
            }
        }
        #endregion ExecuteBulkCopy

        #region ExecuteNonQueryBatch -- by yj 20140924
        /// <summary>
        /// 在同一个连接里执行同一个sp n次,n次中参数值不一样
        /// 或，拼整个sql，一次执行
        /// </summary>
        ...
        /// <returns></returns>
        public static int ExecuteNonQueryBatch(string connectionString, CommandType cmdType, string cmdText, List<List<SqlParameter>> commandParameters)
        {
            if (connectionString == null || connectionString.Length == 0)
                throw new ArgumentNullException("connectionString");
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                foreach (List<SqlParameter> ps in commandParameters)
                {
